...
- 注意：对话历史通常为 系统提示词 → 用户 → 模型 → ... → 模型 → 用户
  - 在结尾插入 `model` 消息意味着请求以模型轮次结束。不同提供商处理不一致：
    - Gemini：不同模型行为不一致。2.5Flash会将这一消息作为用户消息处理并尝试补全（她在以用户的身份输出！），而2.5Pro会在这一消息的基础上开始续写。值得注意的是，这一插入操作疑似会影响思维链的展示，虽然在小部分情况下观测到模型先输出内容再输出思考摘要，但是大部分情况下思考摘要并没有被展示。但是思考本身应当存在，因为观测到了问题难度与首字延时的正相关性。
